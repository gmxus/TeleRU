...
public abstract class BaseLocationAdapter extends RecyclerListView.SelectionAdapter
{
    ...
    public void searchGooglePlacesWithQuery(final String query, final Location coordinate)
    {
        ...
        {
            searching = true;
            final String url = String.format(Locale.US, "https://api.foursquare.com/v2/venues/search/?v=%s&locale=en&limit=25&client_id=%s&client_secret=%s&ll=%s%s",
                    BuildVars.FOURSQUARE_API_VERSION, BuildVars.FOURSQUARE_API_ID, BuildVars.FOURSQUARE_API_KEY,
                    String.format(Locale.US, "%f,%f", coordinate.getLatitude(), coordinate.getLongitude()),
                    query != null && query.length() > 0 ? "&query=" + URLEncoder.encode(query, "UTF-8") : "");
            /*
            GOOGLE MAPS
