...
{
    public static final String TAG = NotificationsController.class.getName();
    ...
    public NotificationsController()
    {
        ...
        {
            notificationChannel = new NotificationChannel(Integer.toString(hashCode()), TAG, NotificationManager.IMPORTANCE_DEFAULT);
    ...
    protected void showSingleBackgroundNotification()
    {
        notificationsQueue.postRunnable(() ->
        {
            try
            {
                ...
                {
                    vibrateOverride = 0;
                    chooseSoundPath = preferences.getString("GlobalSoundPath", defaultPath);
                ...
                if (!notifyDisabled)
                {
                    ...
                    if (chooseSoundPath != null && !chooseSoundPath.equals("NoSound"))
                    {
                        if (chooseSoundPath.equals(defaultPath))
                            mBuilder.setSound(Settings.System.DEFAULT_NOTIFICATION_URI, AudioManager.STREAM_NOTIFICATION);
                        else
                            mBuilder.setSound(Uri.parse(chooseSoundPath), AudioManager.STREAM_NOTIFICATION);
                    }
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
                {
                    Notification.Builder builder = Notification.Builder.recoverBuilder(ApplicationLoader.applicationContext, notification);
    ...
    public void removeDeletedMessagesFromNotifications(final SparseArray<ArrayList<Integer>> deletedMessages)
    {
        ...
        notificationsQueue.postRunnable(() ->
        {
            ...
            if (popupArray != null)
            {
                AndroidUtilities.runOnUIThread(() -> popupMessages = popupArray);
            }
    ...
    private void showExtraNotifications(NotificationCompat.Builder notificationBuilder, boolean notifyAboutLast)
    {
        ...
        for (int b = 0; b < sortedDialogs.size(); b++)
        {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
            {
                Notification.Builder _builder = Notification.Builder.recoverBuilder(ApplicationLoader.applicationContext, notification);
                _builder.setChannelId(notificationChannel.getId());
