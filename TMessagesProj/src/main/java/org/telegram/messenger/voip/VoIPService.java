...
public class VoIPService extends Service implements
        VoIPController.ConnectionStateListener,
        SensorEventListener,
        ...
        NotificationCenter.NotificationCenterDelegate
{
            ...
            else if ((getPackageName() + ".ANSWER_CALL").equals(intent.getAction()))
            {
                if (intent.getIntExtra("end_hash", 0) == endHash)
                {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && checkSelfPermission(Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED)
                    {
                        try
                        {
                            PendingIntent.getActivity(VoIPService.this, 0,
                                    new Intent(VoIPService.this, VoIPPermissionActivity.class)
                    ...
                    {
                        PendingIntent.getActivity(VoIPService.this, 0,
                                new Intent(VoIPService.this, VoIPActivity.class)
    ...
    public void hangUp()
    {
        declineIncomingCall(currentState == STATE_RINGING || (currentState == STATE_WAITING && isOutgoing) ?
                DISCARD_REASON_MISSED : DISCARD_REASON_HANGUP, null);
    }

    public void hangUp(Runnable onDone)
    {
        declineIncomingCall(currentState == STATE_RINGING || (currentState == STATE_WAITING && isOutgoing) ?
                DISCARD_REASON_MISSED : DISCARD_REASON_HANGUP, onDone);
    }
    ...
    private void initiateActualEncryptedCall()
    {
        ...
        try
        {
            ...
            if (prefs.getBoolean("proxy_enabled", false) && prefs.getBoolean("proxy_enabled_calls", false))
            {
                ...
                {
                    controller.setProxy(server,
                            prefs.getInt("proxy_port", 0),
                            prefs.getString("proxy_user", null),
                            prefs.getString("proxy_pass", null));
                }
            }
            else if (ProxyController.getInstance().isAvailable() && ProxyController.getInstance().useForCalls())
            {
                controller.setProxy(ProxyController.getInstance().getHost(), ProxyController.getInstance().getPort(),
                        ProxyController.getInstance().getUsername(), ProxyController.getInstance().getPassword());
            }
