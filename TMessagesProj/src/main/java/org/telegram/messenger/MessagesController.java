...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate
{
    ...
    public void processLoadedMessages(final TLRPC.messages_Messages messagesRes, final long dialog_id, final int count, final int max_id,
                                      final int offset_date, final boolean isCache, final int classGuid, final int first_unread,
                                      final int last_message_id, final int unread_count, final int last_date, final int load_type,
                                      final boolean isChannel, final boolean isEnd, final int loadIndex, final boolean queryFromServer)
    {
            ...
            public void run()
            {
                ...
                if (high_id != 1 && lower_id != 0 && isCache && messagesRes.messages.size() == 0)
                {
                    AndroidUtilities.runOnUIThread(new Runnable()
                    {
                        ...
                        public void run()
                        {
                            loadMessages(dialog_id, count, load_type == 2 && queryFromServer ? first_unread : max_id,
                                    offset_date, false, 0, classGuid, load_type, last_message_id, isChannel,
                                    loadIndex, first_unread, unread_count, last_date, queryFromServer);
                        }
                    });
                    return;
                }
                ...
                AndroidUtilities.runOnUIThread(new Runnable()
                {
                    ...
                    public void run()
                    {
                        ...
                        NotificationCenter.getInstance().postNotificationName(NotificationCenter.messagesDidLoaded, dialog_id, count, objects, isCache,
                                first_unread_final, last_message_id, unread_count, last_date, load_type, isEnd, classGuid, loadIndex, max_id);
...
//    }
        ...
        for (int c = 0; c < updates.size(); c++)
        {
            ...
            else if (update instanceof TLRPC.TL_updateUserTyping || update instanceof TLRPC.TL_updateChatUserTyping)
            {
                if (update.user_id != UserConfig.getClientUserId())
                {
                    ...
                    {
                        AndroidUtilities.runOnUIThread(() -> NotificationCenter.getInstance().postNotificationName(NotificationCenter.showStatusNotifications, id, false));
                    }
                }
            }
                    ...
                    for (int a = 0; a < updatesOnMainThread.size(); a++)
                    {
                        ...
                        else if (update instanceof TLRPC.TL_updateUserPhone)
                        {
                            if (currentUser != null)
                            {
                                ...
                                Utilities.phoneBookQueue.postRunnable(() -> ContactsController.getInstance().addContactToPhoneBook(currentUser, true));
                            }
    ...
    public static void openByUserName(String username, final BaseFragment fragment, final int type)
    {
        ...
        else
        {
            ...
            final int reqId = ConnectionsManager.getInstance().sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() ->
            {
                try
                {
                    progressDialog.dismiss();
